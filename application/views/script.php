...
<script>
	...
	// function showSubstance(headerRow, idHeader) {
	// 	const substanceRows = document.querySelectorAll('.training-row');
	// 	const icon = document.getElementById('icon' + idHeader);
	// 	let contain = false;

	// 	substanceRows.forEach((subRow) => {
	// 		if (subRow.classList.contains(idHeader + '-substance')) {
	// 			subRow.remove();
	// 			contain = true;
	// 			icon?.classList.replace('la-chevron-circle-up', 'la-chevron-circle-down');
	// 		}
	// 	});

	// 	if (!contain && icon != null) {
	// 		icon.classList.replace('la-chevron-circle-down', 'la-chevron-circle-up');
	// 		<?php echo $combinedDataJSON ?>.forEach((substance) => {
	// 			if (substance.id_header === idHeader) {
	// 				const row = document.createElement('tr');
	// 				row.classList.add('training-row', idHeader + '-substance');
	// 				createTextCell('', row, 'text', 'left');
	// 				createTextCell('\u2022\u2003' + substance.title, row, 'text', 'left');
	// 				createFileCell(substance.id_detail, substance.path, 'main', row);
	...
	// async function getTrainingByNPK(isAll, keyword) {
	// 	var xhr = new XMLHttpRequest();
	// 	xhr.onreadystatechange = function() {
	// 		if (xhr.readyState === XMLHttpRequest.DONE) {
	// 			if (xhr.status === 200) {
	...
	// 	xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	// 	xhr.send('isAll=' + encodeURIComponent(isAll) + '&keyword=' + encodeURIComponent(keyword));
	// }
